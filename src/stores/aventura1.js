...
class Aventura {
  constructor(lang = 'en',options) {
    ...
    this.options = {
      ...
      minigifOptions: {},
      sceneCallback: (s) => {
        const spanElements = document.querySelectorAll('.storymeta-key');
  ...
  async setDataScenes(scenes, data = undefined, meta = []) {
    if (typeof d3 === 'undefined') {
      console.error(this.lang === 'es' ? "Para crear visualizaciones debes tener también la librería D3" : "To create visualizations you must have also the MiniGif D3");
      return
    }
    ...
    if (data !== undefined) {
      ...
      await this.setViz(this.scenes, this.data, meta);
    }
  ...
  async setViz(scenes, data) {
    ...
    for (let k of keys) {
      ...
      const {viz, areas} = await vizFunction[type](filtered, x, y, this.options);
      scenes[k].image = viz;
  ...
  goToScene(scene) {
    ...
    if (scene.meta !== undefined) {
      ...
      for (let mk of this.metaKeys) {
        ...
        //cambia las palabras de las columnas que se aportaron en el tercer argumento de setDataScenes
        const labelMapping = {
          obra: 'Obra',
          autor: 'Autor',
          fecha: 'Fecha',
        ...
        const labelText = labelMapping[mk] || mk;
...
async function compareViz(data, h1, h2, options) {
  const filtered = [data.find(d => d.ID == h1), data.find(d => d.ID == h2)];
  const width = options.vizWidth;
  const height = options.vizHeight;
...
async function scatterViz(data, vx, vy, options) {
  const width = options.vizWidth;
  const height = options.vizHeight;

  const size = options.vizImageSize;
  ...
  for (let d of filtered) {
    ...
    img.src = d.IMGURL;
    await new Promise(r => { img.onload = () => { r(true) }});
    imgs.push({ url: d.IMGURL, x: scaleX(d[vx]), y: scaleY(d[vy]), img, r: size, ID: d.ID });
  }
...
async function packViz(data, h1, h2, options) {
  const width = options.vizWidth;
  const height = options.vizHeight;
  ...
  for (let f of filtered) {
    for (let d of root.leaves()) {
      if (f[h1] === d.parent.data[0] && f[h2] === d.data[0]) {
        ...
        img.src = f.IMGURL;
        await new Promise(r => { img.onload = () => { r(true) }});
        imgs.push({ url: f.IMGURL, x: d.x, y: d.y, r: options.vizImageSize, img: img, ID: f.ID });
      }
    }
  }
...
`
export const aventura1 = readable(Aventura);
